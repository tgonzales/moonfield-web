...
export async function createCart(lines: CartLineInput[]): Promise<ShopifyCart> {
  ...
  }>(CREATE_CART_MUTATION, { lines }, { cache: "no-store" });
...
export async function addCartLines(cartId: string, lines: CartLineInput[]): Promise<ShopifyCart> {
  ...
  }>(ADD_CART_LINES_MUTATION, { cartId, lines }, { cache: "no-store" });
...
export async function updateCartLines(
  ...
  }>(UPDATE_CART_LINES_MUTATION, { cartId, lines }, { cache: "no-store" });
...
export async function removeCartLines(cartId: string, lineIds: string[]): Promise<ShopifyCart> {
  ...
  }>(REMOVE_CART_LINES_MUTATION, { cartId, lineIds }, { cache: "no-store" });
